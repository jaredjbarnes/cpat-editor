...
import {
  ...
  Optional,
  Group,
} from './railroad_diagrams/railroad.js';
    ...
    switch (pattern.type) {
      ...
      case 'regex': {
        const diagram = new Diagram(
          new Group(`/${(pattern as Regex).regex}/`, pattern.name)
        );
        diagram.attrs.id = pattern.id;
      ...
      case 'expression': {
        ...
        if (prefixPatterns.length > 0) {
          const prefixOptions = new Optional(
            new OneOrMore(new Choice(0, ...prefixChildren))
          );
        ...
        if (postfixPatterns.length > 0) {
          const postfixOptions = new Optional(
            new OneOrMore(new Choice(0, ...postfixChildren))
          );
