...
export class FileExplorerPresenter {
  ...
  startDirectoryCreation() {
    ...
    this._pendingDirectoryCreation.set(
      new DirectoryCreation(path, this._fileSystem, async () => {
  ...
  startRenamingDirectory(filePath: string) {
    this._pendingDirectoryRenaming.set(
      new DirectoryRenaming(
        filePath,
        this._fileSystem,
        async (filePath: string) => {
