...
export class AppPresenter {
  ...
  constructor() {
    ...
    this._currentPath = new Signal<string | null>(
      window.localStorage.getItem(CURRENT_PATH_SELECTED_KEY)
    );
    ...
    this.grammarEditor = new GrammarEditorPresenter({
      onGrammarProcess: (patterns, tokensMap, refreshSyntaxHighlighting) => {
        this.testEditor.setPatterns(patterns, tokensMap, refreshSyntaxHighlighting);
      },
      onSave: (text: string) => {
        this.save(text);
      },
  ...
  showDebugger() {
    ...
    if (pattern != null) {
      const presenter = new DebuggerPresenter(
        this.testEditor.textEditor.getText(),
        pattern
      );
