...
export class AlternatingSequence extends DiagramMultiContainer {
  ...
  toTextDiagram() {
    ...
    separator.push(
      ' '.repeat(leftWidth - 1) + ' ' + cross_diag + ' ' + ' '.repeat(rightWidth - 2)
    );
    separator.push(
      line.repeat(leftWidth - 1) +
        corner_bot_right +
...
export class HorizontalChoice extends DiagramMultiContainer {
  ...
  format(x, y, width) {
    ...
    var lowerSpan =
      ...
      Options.AR;
    var lowerStart =
      x + Options.AR + first.width + (first.needsSpace ? 20 : 0) + Options.AR * 2;
  ...
  constructor(normal, type, ...items) {
    ...
    for (var i = 0; i < this.items.length; i++) {
      ...
      if (i < normal) {
        ...
        this.down += Math.max(
          minimum,
          item.up + Options.VS + this.items[i - 1].down + this.items[i - 1].height
        );
      }
    }
...
export class Terminal extends FakeSVG {
  ...
  format(x, y, width) {
    ...
    if (this.href) new FakeSVG('a', { 'xlink:href': this.href }, [text]).addTo(this);
  ...
  expand(left, right, top, bottom) {
    ...
    if (left + right + top + bottom === 0) {
      ...
      for (i = 0; i < this.height; i++) {
        ...
        newLines.push(
          leftExpansion.repeat(left) + this.lines[i] + rightExpansion.repeat(right)
        );
      }
      for (i = 0; i < bottom; i++) {
        newLines.push(' '.repeat(this.width + left + right));
