...
export class TestEditorPresenter {
  ...
  initialize() {
    ...
    this._blurListener = this.textEditor.onChange((value: string) => {
      if (
        this._options.patternFilePath == null ||
        this.selectedPatternBroadcast.get() == null
      ) {
        return;
      }
  ...
  selectPattern(name: string | null) {
    const savedQuickTestContent = window.localStorage.getItem(
      `${QUICK_TEST_PREFIX}${this._options.patternFilePath}[${name}]`
    );
    ...
    if (testSuitePresenter != null) {
      testSuitePresenter.setContext(
        this._options.patternFilePath,
        this.selectedPatternBroadcast.get()
      );
    }
  }
  ...
  showTestSuite() {
    if (this._options.patternFilePath != null) {
      this._testSuitePresenter.set(
        new TestSuitePresenter(
          this._options.patternFilePath,
          this.selectedPatternBroadcast.get(),
          value => {
            this._process(value);
